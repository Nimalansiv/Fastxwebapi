    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordDTO forgotDto)
        {
            if (!ModelState.IsValid)
            {
                ...
                return BadRequest(new { status = string.Join("|", errors) });
            }

            try
            {
                var result = await _authenticateService.ForgotPassword(forgotDto);
            ...
            {
                _logger.LogError(e, "Error in ForgotPassword");
                return StatusCode(500, new { message = "An error occurred while resetting password.", error = e.Message });
            }
        }
        ...
        public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordDTO changePasswordDto)
        {
            ...
            try
            {
                var result = await _authenticateService.ChangePassword(changePasswordDto);
            ...
            {
                _logger.LogError(e, "Error in ChangePassword");
                return StatusCode(500, new { message = "An error occurred while changing password.", error = e.Message });
            }
        }
