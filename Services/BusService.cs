...
namespace FastxWebApi.Services
{
    public class BusService:IBusService
    {
        ...
        public BusService(IBusRepository busRepository, IMapper mapper, ISeatRepository seatRepository, ILogger<BusService> logger, IUserRepository userRepository)
        {
        ...
        public async Task<BusDTO> AddBus(BusDTO busDTO,int busOperatorId)
        {
            try
            {
                ...
                //for (int i = 1; i <= busDTO.TotalSeats; i++)
                //{
                //    seatsToAdd.Add(new Seat
                //    {
                //        SeatNumber = $"A{i}",
                //        IsBooked = false,
                //        BusId = addedBus.BusId // Use the ID of the newly added bus
                //    });
                //}

                //await _seatRepository.AddRange(seatsToAdd);
